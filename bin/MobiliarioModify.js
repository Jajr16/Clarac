...
function modifyMob(req, callback) {
    const data = req.body
    console.log(data)
    db.query('UPDATE mobiliario SET Articulo = ?, Descripcion = ?, Ubicacion = ?, Cantidad = ? WHERE Articulo = ? AND Descripcion = ? AND Num_emp = (SELECT Num_emp FROM usuario WHERE Usuario = ?)', [data.Articulo, data.Descripcion, data.Ubicacion, data.Cantidad, data.dataOldA, data.dataOldD, data.user], function (err2, result) {

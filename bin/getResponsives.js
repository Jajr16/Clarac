        ...
        if (err) { Errores(err); return callback(err); } // Se hace un control de errores
            ...
            if (res) {
                ...
                if (data.Responsiva == "MOBILIARIO") {
                    db.query('select*from mobiliario where Num_emp = ? order by Ubicacion;', [num_emp], function (err, res) {
                        if (err) { Errores(err); return callback(err); } // Se hace un control de errores
                        else {
                            if (res) {
                    ...
                    db.query('SELECT DISTINCT Equipo.N_Inventario, Equipo.Num_Serie, Equipo.Equipo, Equipo.Marca, Equipo.Modelo, Equipo.Num_emp, Equipo.Ubi, PCs.Hardware, PCs.Software, Monitor.Num_Serie_CPU, Mouse.Mouse, Teclado.Teclado, Accesorio.Accesorio FROM Equipo LEFT JOIN PCs ON Equipo.Num_Serie = PCs.Num_Serie LEFT JOIN Monitor ON Equipo.Num_Serie = Monitor.Num_Serie_Monitor LEFT JOIN Mouse ON Equipo.Num_Serie = Mouse.Num_Serie LEFT JOIN Teclado ON Equipo.Num_Serie = Teclado.Num_Serie LEFT JOIN Accesorio ON Equipo.Num_Serie = Accesorio.Num_Serie WHERE Num_emp = ? order by Ubi;', [num_emp], function (err, res) {
                        if (err) { Errores(err); return callback(err); } // Se hace un control de errores
                        else {
                            if (res) {
